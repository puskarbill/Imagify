...
const login = () => {
  ...
  // for login and other backend frontend
    ...
    try {
      if(state==='Login'){
        ...
        if(data.success){
          ...
          toast.error(data.message)   // toastify is use for popup
        }
      }else{
  ...
  return (
    ...
    <form onSubmit={onSubmitHandler} className='relative bg-white p-10 rounded-xl text-slate-500'>
            ...
            <div className='border px-6 py-2 flex items-center gap-2 rounded-full mt-4'>
                ...
                <input type='email' onChange={e => setEmail(e.target.value)} value={email} className='outline-none text-sm' placeholder='Gmail ' required/>
            </div>
            ...
            {state=== 'Login' ? <p className='mt-5 text-center'>Dont have an account ? <span className='text-blue-600 cursor-pointer'
